...
=======
  ...
  Matches
    .query({where: {initiator_id: user.attributes.fb_id, matched:1}, 
      orWhere: {target_id: user.attributes.fb_id, matched:1}})
>>>>>>> 71189ece1f002e962c4adf1ecc58e1e9c61f7b50
      .fetch()
      .then(function(matches){
        if(!matches){
          serverUtil.sendResponse(res, 'No matches', 204);
        } else {
          serverUtil.sendResponse(res, matches.models, 200);
        }
      })
      .catch(function(err){
        console.log('Error(serveMatches): ', err);
      });
};
