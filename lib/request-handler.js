...
exports.serveMatches = function(req, res){
  ...
  if(query.apiKey !== API_KEY){
    serverUtil.send404(req, res);
    return;
  } 
  // check that the request provided a valid token. If it's not a valid token
  // then we 404 the request
  // Otherwise we use the token to retrieve the user and return their matches

  var user = new User({access_token: query.access_token})
    .fetch()
    .then(function(user){
      if(!user){
        serverUtil.send404(req, res)
